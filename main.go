...
func main() {
	...
	if client, err = getClient(); err != nil {
		fmt.Errorf("Error in getClient: ", err)
		os.Exit(1)
	}
	if deploymentLabels,expectedPods, err = deploy(ctx, client); err != nil {
		fmt.Printf("Error in Deploy: %v\n", err)
		os.Exit(1)
	}
	if err  = getPods(ctx, client,deploymentLabels,expectedPods);err !=nil{
		fmt.Printf("Error Counting Pods: %v\n", err)
		os.Exit(1)
	}
	
	fmt.Println("Deploy finish", deploymentLabels)
}
...
func getPods(ctx context.Context, client *kubernetes.Clientset, deploymentLabels map[string]string,expectedPods int32) error {
	for {
